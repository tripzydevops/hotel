export const en = {
  common: {
    dashboard: "Dashboard",
    analysis: "Analysis",
    reports: "Reports",
    parityMonitor: "Parity Monitor",
    settings: "Settings",
    alerts: "Alerts",
    profile: "Profile",
    signIn: "Sign In",
    signOut: "Sign Out",
    getStarted: "Get Started",
    loading: "Loading...",
    save: "Save",
    cancel: "Cancel",
    scanNow: "Scan Now",
    scanning: "Scanning...",
    refreshData: "Refresh Market Data",
    addHotel: "Add Hotel",
    myHotel: "My Hotel",
    competitor: "Competitor",
    perNight: "per night",
    upgrade: "Upgrade",
    dismiss: "Dismiss",
    retry: "Retry",
    delete: "Delete",
    edit: "Edit",
    view: "View",
    today: "Today",
    justNow: "Just Now",
    you: "You",
    pending: "Pending",
    availableNow: "Available Now",
    privacy: "Privacy",
    accessibility: "Accessibility",
    terms: "Terms",
    checkIn: "Check-in",
    checkOut: "Check-out",
    loadingError: "Failed to load dashboard data.",
    footerCopyright:
      "© 2026 Hotel Rate Sentinel.",
    close: "Close",
    errorTitle: "Error",
    estimated: "ESTIMATED / SOLD OUT",
    saveSuccess: "Saved successfully",
    scan: "scan",
    scans: "scans",
    latest: "Latest",
    high: "High",
    low: "Low",
    average: "Average",
    intelligencePending: "Deep Intelligence Pending...",
    market: "Mkt",
    yourHotel: "Your Hotel",
    setAsMine: "Set as Mine",
    xOfY: "{current} of {total}",
    found: "Found",
    matching: "matching",
    performance: "Performance",
    above: "Above",
    below: "Below",
    crossPlatformIntelligence: "Cross-Platform Intelligence",
    compare: "Compare",
    collapse: "Collapse",
    cookieConsentTitle: "Cookie Consent",
    cookieConsentText: "We use cookies to optimize your rate intelligence experience, analyze site usage, and support our security compliance under GDPR & CCPA. By clicking accept, you consent to our use of these technologies.",
    cookieAccept: "Accept All",
    cookieDecline: "Reject Non-Essential",
  },
  auth: {
    loginTitle: "Log in to your B2B dashboard",
    signupTitle: "Register for your real-time monitoring account",
    loginSubtitle: "Access your enterprise competitor dashboard",
    signupSubtitle: "Register for your real-time monitoring account",
    emailLabel: "Business Email",
    passwordLabel: "Secure Password",
    signInButton: "Secure Log In",
    signUpButton: "Initialize Account",
    newToPlatform: "New to the platform?",
    alreadyHaveAccount: "Already have an account?",
    requestAccess: "Request Access / Sign Up",
    backToLogin: "Back to Secure Login",
    protectedText: "Automated Market Intel • Enterprise Security Layer",
    errorTitle: "Authentication Error",
    emailPlaceholder: "name@company.com",
    verifyTitle: "Verify your account",
    verifySubtitle: "Enter the 6-digit code sent to {email}",
    otpLabel: "Verification Code",
    verifyButton: "Verify & Dashboard",
    resendButton: "Resend Code",
    wrongEmail: "Entered wrong email?",
    alreadyHaveCode: "Already have a code?",
    pendingTitle: "Access Pending Approval",
    pendingMessage: "Your account has been created successfully. For security reasons, a system administrator must manually verify your identity before you can access the dashboard. Please check back soon or contact support if this takes longer than 24 hours.",
    mfaTitle: "Security Verification",
    mfaDescription: "Please enter the 6-digit verification code from your authenticator application.",
    mfaDescriptionEmail: "Please enter the 6-digit verification code sent to your registered email address.",
    mfaVerifyButton: "Verify Passcode",
    mfaResendButton: "Resend Code via Email",
    mfaResendSuccess: "A new security code has been sent to your email address.",
  },
  dashboard: {
    title: "Rate Intelligence Grid",
    subtitle: "Autonomous Intelligence / Command Center",
    marketPulse: "Market Pulse",
    nextScheduledScan: "Next Scheduled Scan",
    yieldRisk: "Yield Risk Area",
    marketOpportunity: "Market Opportunity",
    avgCompetitor: "Avg Competitor",
    hotelsTracked: "Hotels Tracked",
    undercut: "Undercutting You",
    pricesDropped: "Prices Dropped",
    previous: "Previous",
    change: "Change",
    updated: "Updated",
    liveRate: "Live Market Rate",
    verified: "Verified via Rate Intelligence",
    pendingInitial: "Pending initial scan",
    removeConfirm: "Are you sure you want to remove this hotel monitor?",
    removeSuccess: "Hotel removed from watch list",
    removeError: "Failed to delete hotel monitor",
    searchHistoryTitle: "Google Hotels History",
    scanHistoryTitle: "Scan History",
    rapidPulseTitle: "Rapid Pulse Real-time",
    agentMeshActive: "Agent-Mesh Active",
    upgradedToPlan: "Upgraded to {plan} plan successfully!",
    nextScanNote: "Manual scans reset the scheduled countdown.",
    limitReached: "Hotel Limit Reached",
    liveMarketRate: "Live Market Rate",
    sentimentScore: "Sentiment Score",
    rateParity: "Rate Parity",
    marketSynchronized: "Market Synchronized",
    synchronized: "Synchronized",
    activeScans: "{count} Active Scans",
    priceDiscoveryActive: "Price Discovery Active",
    dataUpdated: "Data Updated",
    noHotelsFound: "No Hotels Tracked",
    addFirstHotel: "Add your first hotel to start monitoring your competitive set.",
    aiInsight: "AI Insight",
    liveAnalysis: "Live Analysis",
    healthIndex: "Health Index",
    addError: "Failed to add hotel monitor",
  },
  hotelDetails: {
    overview: "Overview",
    gallery: "Gallery",
    amenities: "Amenities",
    offers: "Market Offers",
    rooms: "Room Types",
    liveRates: "Live Rates",
    intelSummary: "Intelligence Summary",
    amenitiesCount: "Amenities Tracked",
    offersCount: "Market Offers",
    imagesCount: "Images Captured",
    foundVia: "Found via",
    vendor: "Vendor",
    price: "Price",
    diff: "Diff",
    lockedTitle: "{0} is Locked",
    lockedDesc: "Upgrade to Enterprise to unlock full market intelligence.",
    unlockButton: "Unlock Enterprise Features",
    visualIntel: "Visual Intelligence",
    visualIntelDesc:
      "Access high-resolution image galleries to analyze competitor presentation.",
    featureAnalysis: "Feature Analysis",
    featureAnalysisDesc: "Deep dive into competitor amenities and facilities.",
    marketDepth: "Market Depth",
    marketDepthDesc:
      "See full competitor pricing across multiple vendors (Expedia, Booking, Agoda, etc.).",
    noImages: "No images captured for this hotel.",
    noAmenities: "No amenities captured for this hotel.",
    noOffers: "No additional offers found in this scan.",
    noRooms: "No specific room type data available for this date.",
    checkIn: "Stay Date",
    current: "Current",
    marketPresence: "Market Presence",
    trustSummary: "Trust Summary",
    sentimentVoices: "Sentiment Voices",
    stars: "{0} Stars",
    optimalPrice: "Optimal Price",
    locationUnknown: "Location Unknown",
    leadRate: "Lead Rate",
    recency: "Recency",
    pendingSync: "Pending Sync",
    shift: "Price Shift",
    tacticalIntel: "Tactical Intelligence",
    setAsMyHotel: "Set as My Hotel",
    editIntel: "Edit Intel",
    purgeIntel: "Purge Intel",
    ratingDistribution: "Rating Distribution",
    contactInfo: "Contact Details",
    description: "Intelligence Brief",
  },
  addHotel: {
    title: "Add New Hotel",
    nameLabel: "Hotel Name",
    namePlaceholder: "e.g. Grand Plaza Hotel",
    locationLabel: "Location",
    locationPlaceholder: "e.g. Miami, FL",
    currencyLabel: "Currency",
    targetLabel: "This is My Hotel",
    submitButton: "Add Hotel Monitor",
    limitReached: "Hotel Limit Reached (Max {0})",
    limitDesc: "Upgrade to add more hotels.",
    globalMatch: "Global",
    noMatch: "No database matches",
    checkingLocation: "Checking location below to set manually",
    countryLabel: "Country",
    cityLabel: "City",
    addNewLocation: "Add New Location",
    selectCountry: "Select Country",
    selectCity: "Select City",
    manualEntry: "Manual Entry",
  },
  editHotel: {
    title: "Edit Hotel",
    nameLabel: "Hotel Name",
    locationLabel: "Location",
    currencyLabel: "Currency",
    targetLabel: "This is My Hotel",
    submitButton: "Save Changes",
    defaultScanSettings: "Default Scan Settings (Optional)",
    adults: "Adults",
    fixedCheckIn: "Fixed Check-in",
    fixedCheckOut: "Fixed Check-out",
    linkedToGoogle: "Linked to Google Graph ID",
  },
  scanSettings: {
    title: "Scan Settings",
    checkIn: "Check-in",
    checkOut: "Check-out",
    adults: "Guests (Adults)",
    adultCount: "{0} Adult{1}",
    nightCount: "{0} Night{1}",
    startScan: "Start Scan",
    defaultDatesNote: "Leave dates empty for default (Today/Tomorrow).",
  },
  scanSession: {
    title: "Pulse Intelligence",
    averageRate: "Average Rate",
    successRate: "Success Rate",
    vendors: "Vendors",
    sessionId: "Session ID",
    extractingData: "Extracting pulse data...",
    noRecords: "No detailed records found for this session",
    liveRate: "Live Rate",
    verifiedSerp: "Verified intelligence data",
    generatedAt: "Generated {0}",
    csvExport: "CSV",
    hotelName: "Hotel Name",
    vendor: "Vendor",
    location: "Location",
  },
  history: {
    intelLogs: "Intelligence Logs",
    lastPulseCount: "Last {0} Pulses",
    scanSession: "{0} Scan Session",
    hotelsCount: "{0} Hotels",
    sessionIdShort: "Session ID: {0}",
    searchHistory: "Google Hotels History",
    recapQueries: "Recap your intelligence queries",
    reSearch: "Re-search Google Hotels",
    rapidPulseHistory: "Rapid Pulse History",
    propertiesBatch: "{0} Properties Batch",
    viewDetails: "View Details",
  },
  analysis: {
    title: "Market Intelligence",
    tabs: {
      overview: "Overview",
      calendar: "Rate Calendar",
      discovery: "Discovery Engine",
      sentiment: "Sentiment",
    },
    subtitle: "Real-time competitor spread and predictive price analysis.",
    experienceCore: "Experience Core",
    experienceCoreDesc: "Category intelligence across hospitality domains.",
    sentimentSpread: "Sentiment Spread",
    sentimentSpreadDesc: "Historical ratings distribution",
    marketAverage: "Market Average",
    marketSpread: "Market Spread",
    marketPosition: "Market Position",
    inventoryRange: "Inventory Range",
    ofHotels: "of {count} hotels",
    priceLeaderboard: "Price Leaderboard",
    marketComparison: "Market Comparison",
    priceRangeAnalysis: "Price Range Analysis",
    metricPerspective: "Metric Perspective",
    targetPriceDesc: "The ideal price to achieve maximum occupancy and revenue based on current market data and competitive positioning.",
    ari: {
      title: "Avg Rate Index (ARI)",
      subtitle: "Market Rate Competitiveness",
      description: "ARI measures your price position relative to the market median. 100% means you are perfectly aligned with the market average.",
    },
    sentiment: {
      title: "Sentiment Index",
      subtitle: "Reputation vs Market",
      description: "This index compares your guest review scores against the weighted market average to determine your quality-price parity.",
    },
    targetPriceTrend: "My Hotel Price Trend",
    targetPriceTrendDesc: "30-day historical movements",
    historyRepo: "Detailed History Repo",
    maxLabel: "Max",
    minLabel: "Min",
    youLabel: "YOU",
    processing: "Processing Intelligence...",
    system: "System",
    advisory: {
      title: "Agent Intelligence Advisory",
      generatedBy: "Generated by Autonomous Analyst Agent • Real-time Guidance",
      premium:
        "Strategic Peak: Your premium pricing is well-supported by superior guest sentiment.",
      value:
        "Expansion Opportunity: You offer the best value to price ratio in the market.",
      danger:
        "Caution: Your pricing is currently higher than supported by market guest sentiment.",
      volume:
        "Volume Strategy: Your low rates are attracting budget-conscious guests.",
      reputation_focus: "Focus on reputation management.",
      aggressive_monitoring: "Monitor competitors for aggressive price cuts.",
    },
    stability: "Market Stability",
    fieldIntel: "Field Intel",
    marketSentiment: "Market Sentiment",
    dataAlert: "DATA ALERT",
    aiAdvisoryNote: "AI-generated advisory based on current market trends.",
    active: "Active",
    targetPrice: "My Hotel Price",
    objectiveSummary: "Objective Summary",
    strategicInsights: "Strategic Insights",
    analysisConfidence: "Analysis",
    optimal: "Optimal",
    moderate: "Moderate",
    volatile: "Volatile",
    unknown: "Unknown",
    analyzingContext: "Analyzing market context...",
    matchingHotels: 'Found {count} hotels matching "{query}"',
    clearFilter: "Clear Filter",
    ranking: {
      cheapest: "You're the cheapest in the market!",
      excellent: "Excellent competitive position",
      midRange: "Mid-range pricing",
      premium: "Premium pricing tier",
    },
    errors: {
      sessionExpired: {
        title: "Session Expired",
        message: "Your authorization token has timed out. Please log back in to continue your analysis.",
      },
      analysisError: {
        title: "Analysis Error",
        subtitle: "Synchronization issue detected",
        message: "We encountered a problem synchronizing your market data. This may be due to a connection timeout or an expired session.",
      },
      connectionFailed: {
        title: "Connection Failed",
        message: "An unexpected error occurred while connecting to the intelligence engine.",
      },
    },
    gapToMedian: "Gap to Median",
    cheapestLabel: "Cheapest",
    midRangeLabel: "Mid-Range",
    premiumLabel: "Premium",
    baseMetric: "Base Metric",
    marketMinimum: "Market Minimum",
    marketMaximum: "Market Maximum",
    priceGapToMin: "Price Gap to Min",
    inventorySpread: "Inventory Spread",
  },
  reports: {
    title: "Audit & Intelligence Reports",
    subtitle: "Historical scan logs and exportable intelligence summaries.",
    weeklySummary: "Weekly Summary",
    totalPulses: "Total Pulse Scans",
    activeIntelligence: "Active Intelligence",
    weeklyTrend: "Weekly Trend",
    systemHealth: "System Health",
    fullHistoryLog: "Full Intelligence History",
    recent20Sessions: "Displaying recent 20 sessions",
    hotelsScanned: "Hotels Scanned",
    propertiesLabel: "Properties",
    exportPdf: "Export PDF",
    exportCsv: "Export CSV",
    exporting: "Exporting...",
    compiling: "Compiling Audit Logs...",
    timestamp: "Timestamp",
    sessionType: "Session Type",
    status: "Status",
    actions: "Actions",
    marketPosition: "Market Position",
    positionDesc: "Your pricing relative to market extremes.",
    marketMin: "Min",
    myHotel: "You",
    marketAvg: "Avg",
    marketMax: "Max",
    priceVelocity: "Price Velocity",
    velocityDesc: "Historical price trend over last 30 scans.",
    competitiveRank: "Competitive Rank",
    rankMsg: "Ranked #{0} of {1} (1 = Cheapest)",
    noData: "Not enough data",
    marketComparison: "Market Comparison",
    proTip: "Pro Insight",
    noHistoryData: "No historical data available for this period",
    lowPosition:
      "You are positioned below the market average. Consider raising rates to capture more yield.",
    highPosition:
      "You are positioned above the market average. Monitor occupancy levels closely.",
    highVolatility: "High market volatility detected. Review rates daily.",
    lowVolatility:
      "Market variances are low. Stable pricing strategy recommended.",
    currencyLabel: "Currency",
    strategicIntelligence: "Strategic Intelligence",
    ariLabel: "Average Rate Index (ARI)",
    sentimentLabel: "Sentiment Index (GRI)",
    ariDesc: "Your price relative to market average (100 = Fair)",
    sentimentDesc: "Your guest satisfaction relative to market",
    quadrantTitle: "Autonomous Quadrant Advisor",
    quadrantDesc: "AI positioning analysis based on Price vs. Value",
    valuePremium: "Value Premium",
    underpriced: "Underpriced",
    overpriced: "Overpriced",
    lostValue: "Value Gap",
    soWhat: "So What?",
    stealthPattern: "Stealth Pattern Detected",
    yourRate: "Your Rate",
    marketSpread: "Market Spread",
    weakestArea: "Weakest Area",
    yourEdge: "Your Edge",
  },
  subscription: {
    title: "Subscription Plans",
    subtitle:
      "Choose the plan that fits your growth. You are currently on {0}.",
    mostPopular: "Most Popular",
    currentPlan: "Current Plan",
    upgrade: "Upgrade",
    requestQuote: "Contact Sales",
    contactSales:
      "Needs enterprise-grade compliance? Contact Sales for volume licensing.",
    // Plan-specific keys used by SubscriptionModal.tsx
    price: {
      starter: "Contact Sales for Quote",
      pro: "Contact Sales for Quote",
      enterprise: "Custom Quote",
    },
    period: {
      mo: "",
      custom: "",
    },
    starter: {
      name: "Starter",
      description:
        "Perfect for small hotels getting started with rate monitoring.",
      features: {
        0: "Up to 5 hotels tracked",
        1: "Daily scheduled scans",
        2: "5 manual scans / day",
        3: "Basic competitor analysis",
        4: "Email alerts",
      },
    },
    pro: {
      name: "Professional",
      description: "Advanced intelligence for growing hotel chains.",
      features: {
        0: "Up to 25 hotels tracked",
        1: "Hourly scheduled scans",
        2: "8 manual scans / day",
        3: "AI-powered competitor insights",
        4: "Multi-channel alerts",
        5: "Revenue optimization reports",
      },
    },
    enterprise: {
      name: "Enterprise",
      description: "Full-scale intelligence for hotel groups and chains.",
      features: {
        0: "Up to 100 hotels tracked",
        1: "Hourly scheduled scans",
        2: "10 manual scans / day",
        3: "Custom AI agent workflows",
        4: "Dedicated account manager",
        5: "API access & integrations",
      },
    },
  },
  alerts: {
    title: "Alert Center",
    subtitle: "Real-time market intelligence",
    clearAll: "Clear All",
    loading: "Intercepting Signals...",
    emptyTitle: "All Clear",
    emptyDesc: "No critical market changes detected yet.",
    undercut: "COMPETITOR UNDERCUT",
    priceChange: "PRICE CHANGE",
    previous: "Previous",
    current: "Current",
    markAsRead: "Mark as read",
  },
  userMenu: {
    myProfile: "My Profile",
    alertSettings: "Alert Settings",
    adminPanel: "Admin Panel",
    billing: "Billing",
    signOut: "Sign Out",
    upgradeToPro: "Upgrade to Pro",
    planActive: "Active",
    hotelsTracked: "Hotels Tracked",
    planSuffix: "Plan",
  },
  profile: {
    title: "My Profile",
    displayName: "Display Name",
    companyName: "Company / Hotel Name",
    jobTitle: "Job Title",
    phone: "Phone Number",
    timezone: "Timezone",
    saveProfile: "Save Profile",
    saving: "Saving...",
    loading: "Syncing User Profile...",
    error: "Failed to load profile data",
    namePlaceholder: "Your name",
    companyPlaceholder: "Your hotel or company",
    jobPlaceholder: "e.g. Revenue Manager",
    phonePlaceholder: "+1 234 567 8900",
    saveSuccess: "Profile updated successfully",
    privacyTitle: "Privacy & Data",
    privacyDescription: "You have the right to export or delete your personal data at any time in accordance with GDPR and KVKK regulations.",
    exportData: "Export Personal Data",
    exportDataDesc: "Download a copy of all your personal data stored on our platform.",
    deleteAccount: "Permanently Delete Account",
    deleteAccountDesc: "This action is irreversible. All your personal data will be permanently removed.",
    deleteConfirmPrompt: "Type DELETE to confirm",
    deleteConfirmButton: "Confirm Deletion",
    exportSuccess: "Your data export has been downloaded.",
    deleteSuccess: "Your account has been permanently deleted.",
  },
  settings: {
    title: "Alert Settings",
    triggerThreshold: "Trigger Threshold",
    thresholdDesc: "Notify me when rates change by more than {0}%",
    notificationChannels: "Notification Channels",
    emailAlerts: "Email Alerts",
    whatsappAlerts: "WhatsApp Alerts",
    pushAlerts: "Push Notifications",
    savePreferences: "Save Preferences",
    pushEnabled: "Push Notifications Enabled!",
    comingSoon: "Coming Soon",
  },
  sentiment: {
    title: "Sentiment Comparison",
    subtitle: "Benchmarking your property against the local competitive set.",
    backToOverview: "Back to Overview",
    comparingWith: "Comparing with:",
    noDataAvailable:
      "No hotel data available. Please add your hotel first.",
    trendTitle: "6-Month Sentiment Trend",
    categoryBreakdown: "Category Breakdown",
    guestMentions: "Guest Mentions",
    aiInsight: "AI Insight",
    viewAll: "View All",
    eventDetected: "Event Detected",
    cleanliness: "Cleanliness",
    service: "Service",
    location: "Location",
    myHotel: "My Hotel",
    leader: "Leader",
    avgComp: "Avg. Comp.",
    rankSuffix: {
      st: "st",
      nd: "nd",
      rd: "rd",
      th: "th",
    },
    performancePulse: "Performance Pulse",
    sentimentVoices: "Sentiment Voices",
    agentIntelligence: "Agent Intelligence",
    competitiveVulnerabilities: "Competitive Vulnerabilities",
    vulnerabilityDesc:
      "Areas where competitors are weak and you can capitalize.",
    topDriver: "Top Driver",
    reasoningLogic: "Reasoning Logic",
    extracted: "Extracted",
    positiveSignals: "positive signals",
    noMentions: "No specific guest mentions captured yet.",
    multiple: "multiple",
    threatDetected: "Threat Detected",
    opportunity: "Opportunity",
    high: "High",
    guestComplaint: "Guest Complaint",
    noVulnerabilities: "No critical vulnerabilities detected.",
    low: "Low",
    medium: "Medium",
    secure: "Secure",
    competitor: "Competitor",
    overallRating: "Overall Rating",
    currentPrice: "Current Price",
    value: "Value",
  },
  landing: {
    nav: {
      home: "Home",
      about: "About",
      pricing: "Pricing",
      contact: "Contact",
      demo: "Request Demo",
    },
    table: {
      date: "Date",
      yourHotel: "YOUR HOTEL",
      compA: "Comp A",
      compB: "Comp B",
      compC: "Comp C",
    },
    common: {
      login: "Login",
      signup: "Sign Up",
    },
    footer: {
      tagline:
        "AI-powered hotel price monitoring and competitive analysis platform.",
      pages: "Pages",
      product: "Product",
      contact: "Contact",
      rights: "© 2026 Hotel Plus. All rights reserved.",
    },
    hero: {
      topLabel: "Autonomous Market Intelligence",
      titleMain: "The B2B Rate Sentinel for",
      titleHighlight: "Hospitality",
      titleSuffix: "Leaders",
      description:
        "Hotel Plus is powered by the Hotel Rate Sentinel (Enterprise Core). Secure your rate parity, forecast regional demand events, and automate competitive analysis with an otonom Agent Mesh.",
      ctaPrimary: "Request B2B Access",
      ctaSecondary: "View Membership Plans",
    },
    stats: {
      activeHotels: "Active Hotels",
      scans: "Scanning",
      accuracy: "Accuracy Rate",
      monitored: "Monitored Prices",
    },
    features: {
      title: "Technology Designed for Revenue Leaders",
      subtitle: "Platform Capabilities",
      items: {
        priceIntel: {
          title: "Price Intelligence",
          desc: "Scan competitor hotel prices 24/7 in the background, analyze historical trends, and react instantly to pricing shifts.",
        },
        discovery: {
          title: "Discovery Engine",
          desc: "Discover competitor hotels on an interactive map. Powered by pgvector Semantic Search for automated compset matching.",
        },
        parity: {
          title: "Parity Monitor",
          desc: "Instantly detect price inconsistencies across major OTA channels to protect your direct bookings and brand margin.",
        },
        sentiment: {
          title: "Smart Memory Sentiment",
          desc: "Cumulative review tracking with cross-language category normalization to pinpoint competitor vulnerabilities.",
        },
        alerts: {
          title: "Instant Alerts",
          desc: "Receive proactive mobile push and email notifications when competitor rate thresholds are crossed.",
        },
        events: {
          title: "Event & Fair Radar",
          desc: "Track localized fair, festival, and congress data via integrated event streams to capitalize on occupancy compression before competitors sell out.",
        },
        reports: {
          title: "Smart Reports",
          desc: "Empower management decisions with AI-powered market analysis reports and boardroom-ready PDF briefs.",
        },
      },
    },
    personas: {
      title: "Tailored to Your Strategic Role",
      subtitle: "Enterprise Value Props",
      owner: {
        tag: "Hotel Owners & Investors",
        title: "Maximize RevPAR & Margins",
        desc: "Stop revenue leakage from OTA undercutting. Reclaim direct bookings, defend your asset value, and increase yields with real-time rate enforcement.",
      },
      gm: {
        tag: "General Managers (GMs)",
        title: "Boardroom-Ready in 1-Click",
        desc: "Automate manual Excel competitor tracking. Run automated background scans and compile professional 'Meeting Prep' briefs in seconds.",
      },
      revenue: {
        tag: "Revenue Directors & Chains",
        title: "Cognitive Multi-Property Scaling",
        desc: "Solve the 'Cold Start' problem for new properties with pgvector mapping. Model strategic shifts with the What-If simulation engine.",
      },
    },
    testimonials: {
      title: "What Hoteliers Say?",
      subtitle: "Success Stories",
      items: {
        0: {
          quote:
            "We increased our revenue by 15% with Hotel Plus. Competitor analysis helped us avoid offering the right price at the wrong time.",
          author: "Ahmet Y.",
          role: "General Manager, Resort Hotel",
          initials: "AY",
        },
        1: {
          quote:
            "Setup took only 5 minutes. It's great to get rid of complex excel sheets and see all market data on a single screen.",
          author: "Zeynep K.",
          role: "Revenue Manager",
          initials: "ZK",
        },
        2: {
          quote:
            "We got ROI in the first month. We can instantly see competitors' price moves and take action.",
          author: "Mehmet S.",
          role: "Hotel Owner",
          initials: "MS",
        },
      },
    },
    pricing: {
      title: "Choose the Plan That Fits Your Hotel",
      subtitle: "Pricing",
    },
    faq: {
      title: "Frequently Asked Questions",
      subtitle: "We answer your questions.",
      items: {
        0: {
          q: "How long does setup take? Do I need technical knowledge?",
          a: "No, no technical knowledge is required. It takes only 5 minutes to add your hotel to the system. You enter your hotel name, our AI does the rest.",
        },
        1: {
          q: "Which sites do you fetch prices from?",
          a: "We scan all major OTA channels and meta-search engines, including Booking.com, Expedia, Hotels.com, Google Hotels, and direct brand websites.",
        },
        2: {
          q: "Is there a commitment?",
          a: "No, there are no long-term contracts or commitments. You can cancel at any time, only pay for what you use.",
        },
        3: {
          q: "Can I track my own hotel as well?",
          a: "Absolutely. You can see your own prices compared to competitors on a single screen and instantly detect parity issues.",
        },
      },
    },
    footerCta: {
      title: "Strengthen Your Pricing Strategy Today",
      titleHighlight: "Today",
      description:
        "Experience Hotel Plus with a free demo. No credit card required, 5-minute setup.",
      ctaPrimary: "Request B2B Access",
      ctaSecondary: "Already have an account? Log In",
    },
  },
  strategicMap: {
    title: "Strategic Map",
    advisorQuadrant: "Advisor Quadrant",
    currentPosition: "Current Position",
    marketInsight: "Market Insight",
    sentimentIndex: "Sentiment Index",
    priceIndex: "Price Index (ARI)",
    valueIndex: "Value Index",
    positions: {
      valueLeader: {
        label: "Value Leader",
        insight: "Optimal position with strong value perception",
        action: "Maintain quality, explore upsell opportunities",
        description: "Your hotel delivers excellent guest satisfaction at a competitive price point. Guests perceive high value for money compared to alternatives in your market. This is the strongest strategic position — protect it by maintaining service quality while exploring premium upsell opportunities.",
      },
      premiumKing: {
        label: "Premium King",
        insight: "High price justified by superior reputation",
        action: "Protect your brand premium, invest in quality",
        description: "You command premium pricing and your guest ratings justify it. Your reputation acts as a moat — guests willingly pay more because they trust the experience. Continue investing in quality to sustain this advantage, and monitor competitor moves closely to stay ahead.",
      },
      budget: {
        label: "Budget / Economy",
        insight: "Competitive pricing with room to grow",
        action: "Test incremental price increases alongside small upgrades",
        description: "Your pricing is below market average while guest perception is moderate. This could indicate untapped pricing power — guests may be willing to pay more. Consider incremental price increases paired with small experience upgrades to move toward the Value Leader quadrant.",
      },
      dangerZone: {
        label: "Danger Zone",
        insight: "High price with lower perceived value",
        action: "Address weak areas below or adjust pricing to match perception",
        description: "Your rates are above market average but guest satisfaction trails behind competitors. This mismatch creates churn risk — guests feel they're overpaying. Prioritize addressing the weakest sentiment categories (check Experience Core below) or consider a rate adjustment to realign value perception.",
      },
      standard: {
        label: "Standard",
        insight: "Neutral market position",
        action: "Improve your weakest areas below to stand out from the market",
        description: "Your hotel sits near the market average on both price and guest perception. While stable, this position lacks differentiation — you're competing on the same terms as everyone else. Focus on improving the weakest Experience Core categories below to carve out a competitive edge.",
      },
      insufficientData: {
        label: "Insufficient Data",
        insight: "Analysis pending more competitive data",
        action: "Add more competitors and run a scan to populate this analysis",
        description: "We don't have enough competitor data yet to accurately place your hotel on the strategic map. Add more hotels to your tracking list and run a scan to populate this analysis with meaningful market positioning data.",
      },
    },
    strategicLogic: "Strategic Logic",
    strategicLogicDesc: "This matrix cross-references your Price Index (ARI) against your Value Index (Guest Satisfaction) to determine your current market territory.",
    quadrantLabels: {
      danger: "Danger",
      premium: "Premium",
      economy: "Economy",
      value: "Value",
    },
  },
  intradayStory: {
    modalTitle: "AI Intraday Story",
    monitoring: "INTRA-DAY MONITORING",
    rapidPulseActive: "RAPID PULSE ACTIVE",
    priceMovement: "Price Movement (24h)",
    otaInformation: "OTA Information",
    narrativeIntelligence: "NARRATIVE INTELLIGENCE",
    sessionAudit: "SESSION AUDIT",
    agentMesh: "AUTONOMOUS AGENT MESH",
    systemStatus: "SYSTEM STATUS: OPERATIONAL",
    price: "Price",
    diff: "Diff",
    action: "Action",
    time: "Time",
    viewAiStory: "View AI Intraday Story",
    via: "via",
    justNow: "Just now",
    labels: {
      "Price Scan": "Price Scan",
    },
  },
  rateIntelligence: {
    title: "Rate Intelligence Grid",
    subtitle: "14-day price comparison vs compset",
    daysCount: "{count} Days",
    dateRange: "Date Range",
    lowestRate: "Lowest Rate",
    notAvailable: "Price not available for this room type",
    bestPosition: "Best Market Position",
    aboveAvg: "Above Market Avg",
    belowAvg: "Below Market Avg",
    viewDetail: "Rate intelligence detail",
  },
  hotelIntelligence: {
    marketLeader: "Market Leader",
    exportIntelligence: "Export Intelligence",
    bestMarketPrice: "Best Market Price",
    globalSentiment: "Global Sentiment",
    parityScore: "Parity Score",
    activeOTAs: "Active OTAs",
    monitoringGlobal: "Monitoring global channels",
    otaComparison: "OTA Market Comparison",
    otaComparisonDesc: "Real-time price distribution across major booking channels.",
    intelligenceBreakdown: "Intelligence Breakdown",
    intelligenceBreakdownDesc: "Deep dive into service quality and room types.",
    sentimentByCategory: "Sentiment by Category",
    roomTypeDiscovery: "Room Type Discovery",
    verifiedSources: "Verified Sources",
    newParityOpportunity: "New Parity Opportunity",
    analyzeOpportunity: "Analyze Opportunity",
    liveView: "LIVE VIEW",
    scanComplete: "Intelligence Scan Complete",
    opportunityDesc: "Hotels.com just increased prices. Consider a 5% promotional adjustment to capture search volume.",
    interiorOverview: "{hotel} - Interior Overview",
    discoveredVia: "Discovered via high-resolution scraping of global OTAs."
  },
  aboutPage: {
    topLabel: "ABOUT US",
    titleMain: "The Rate Data",
    titleHighlight: "Revolution",
    desc: "Hotel Plus was established to make price benchmarking and yield management 100% data-driven. Using advanced otonom agents and real-time market streams, we help hotels maximize RevPAR and GOPPAR safely.",
    whyTitle: "Why Choose",
    whyHighlight: "Hotel Plus",
    whyDesc1: "Hotel pricing is dynamically volatile. Manually tracking dozens of competitor OTA channels and fluctuating local demand compression is practically impossible for on-property teams.",
    whyDesc2: "Hotel Plus automates the entire loop via otonom background scanners. We audit rates 24/7, analyze local event spikes, and synthesize dynamic strategist advisories so you can focus on execution.",
    statLabel: "Average Revenue Increase — across hotels using Hotel Plus",
    setupTime: "3 Min",
    setupLabel: "Setup Time",
    autoScans: "24/7",
    autoLabel: "Scans Running",
    howItWorks: "How It Works",
    howItWorksSubtitle: "Deploy in 3 Simple Steps",
    step1Title: "Add Your Property",
    step1Desc: "Type in your hotel's name and location. Our pgvector cognitive engine automatically populates your active competitor set and OTA links.",
    step2Title: "AI Scanning Dispatches",
    step2Desc: "Our background scheduler triggers regional rate audits 24/7, capturing room-type mappings and matching direct vs OTA rates.",
    step3Title: "Execute Dynamic Strategy",
    step3Desc: "Gain boardroom-ready briefs and proactive notifications. Command the market with accurate event and parity data.",
    ctaTitle: "Experience It",
    ctaHighlight: "Today",
    ctaDesc: "Discover the yield recovery power of Hotel Plus with a custom demo.",
    ctaButton: "Request a Free B2B Demo",
  },
  pricingPage: {
    topLabel: "PRICING",
    titleMain: "Transparent &",
    titleHighlight: "Fair Plans",
    desc: "B2B plans tailored to your property size and operational strategy. 14-day free trial, no credit card required.",
    comparisonTitle: "Detailed Capability Matrix",
    featureCol: "Platform Capability",
    objectionTitle: "Still Undecided?",
    objectionDesc: "Try Hotel Plus risk-free with a customized on-property demonstration.",
    objectionCta: "Request Free B2B Demo",
    faqTitle: "Frequently Asked Questions",
    faqSubtitle: "We address common objection questions.",
    planStarterDesc: "Basic competitor monitoring for small boutique hotels.",
    planProDesc: "Advanced real-time intelligence for yield managers.",
    planEntDesc: "Custom multi-property workflows for hotel chains.",
    objections: {
      0: {
        q: "How long does setup take?",
        a: "Under 3 minutes. Since the Sentinel Core is pre-integrated, you enter your hotel name and our pgvector search builds the rest automatically.",
      },
      1: {
        q: "What booking channels do you track?",
        a: "We scan all major OTA channels and meta-search engines, including Booking.com, Expedia, Hotels.com, Google Hotels, and direct brand websites.",
      },
      2: {
        q: "Is there a contract commitment?",
        a: "No. You pay month-to-month and can upgrade, downgrade, or cancel at any time with no penalties.",
      },
      3: {
        q: "How is my data secured?",
        a: "All personal identifiers and billing scopes are strictly segregated. Our regional InsForge storage is fully SOC 2 and GDPR compliant.",
      },
      4: {
        q: "Can I customize alerts?",
        a: "Yes. GMs can configure specific percentage thresholds and direct push, Slack, or email notifications for rate cuts.",
      },
    },
    roi: {
      title: "Interactive Revenue Recovery Calculator",
      subtitle: "Estimate Your Yield Optimization Lift",
      roomsLabel: "Number of Rooms",
      adrLabel: "Average Daily Rate (ADR)",
      estMonthly: "Estimated Monthly Revenue Recovered",
      roiMultiple: "Expected Return on Investment (ROI)",
      multipleValue: "{val}x Yield ROI",
      disclaimer: "Based on our audited client median performance: a conservative 3.8% direct booking and rate-parity RevPAR lift on a standard 70% baseline occupancy.",
    },
  },
  contactPage: {
    topLabel: "CONTACT",
    titleMain: "Request Your B2B",
    titleHighlight: "Demo Briefing",
    desc: "Experience how Hotel Plus can recover parity leaks and optimize RevPAR on your property. Our B2B onboarding team will contact you in under 24 hours.",
    emailLabel: "Business Email",
    addressLabel: "Headquarters",
    addressValue: "Balikesir, Turkey",
    form: {
      name: "Full Name",
      namePlaceholder: "Your First and Last Name",
      email: "Business Email",
      emailPlaceholder: "name@company.com",
      hotel: "Hotel Name",
      hotelPlaceholder: "e.g., Grand Plaza Hotel",
      message: "Message / Specific Needs (Optional)",
      messagePlaceholder: "Let us know about your room capacity or target competitor set...",
      submit: "Submit Request",
      successTitle: "Request Received!",
      successDesc: "Our hospitality onboarding team will contact you shortly to schedule your live demo.",
      error: "An unexpected error occurred. Please try again or email us directly.",
    },
  }
};
