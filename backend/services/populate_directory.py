...
def populate_hotel_directory(
    cities: List[str], limit_per_city: int = 20, star_rating: int = None
):
    """
    ...
    for city in cities:
        print(f"\n{'=' * 50}")
        print(
            f"Scraping {star_rating if star_rating else ''} star hotels in {city}, Turkey..."
        )
        print("=" * 50)
        ...
        try:
            ...
            for hotel in hotels[:limit_per_city]:
                ...
                hotel_data = {"name": hotel.name, "location": f"{city}, Turkey"}
...
def main():
    ...
    if not SUPABASE_KEY:
        ...
        populate_hotel_directory(
            test_cities, limit_per_city=20, star_rating=star_rating
        )
