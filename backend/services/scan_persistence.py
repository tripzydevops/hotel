...
class ScanPersistenceService:
    ...
    async def update_room_type_catalog(
        ...
        if valid_upserts:
            try:
                ...
                self.admin_db.table("room_type_catalog").upsert(
    ...
    async def sync_from_external_provider(
        ...
        try:
            ...
            if hotel_update:
                self.admin_db.table("hotels").update(hotel_update).eq(
                    "id", hotel_id
                ).execute()
            ...
            if result.get("sentiment_breakdown"):
                ...
                self.admin_db.table("sentiment_history").insert(
                    sentiment_entry
                ).execute()
    ...
    async def _process_hotel_entry(
        ...
        if meta_update:
            try:
                self.admin_db.table("hotels").update(meta_update).eq(
                    "id", hotel_id
                ).execute()
            except Exception as e:
    ...
    async def _process_embeddings(self, queue, session_id, log_reasoning_fn):
        ...
        for i, res in enumerate(results):
            ...
            self.admin_db.table("hotels").update({"embedding_status": status}).eq(
                "id", hid
            ).execute()
