...
class RapidApiProvider(HotelDataProvider):
    ...
    async def fetch_price(
        ...
        try:
            async with httpx.AsyncClient() as client:
                # 1. Resolve Hotel ID
                dest_id, search_type = await self._resolve_destination(
                    client, hotel_name, location
                )
    ...
    async def _resolve_destination(
        self, client: httpx.AsyncClient, hotel_name: str, location: str
        ...
        async def perform_search(search_query: str):
            try:
                resp = await client.get(
                    url, headers=self.headers, params={"query": search_query}
                )
                ...
                print(
                    f"[RapidAPI] Destination search exception for '{search_query}': {e}"
                )
                return None, None
    ...
    async def _get_hotel_details(
        ...
        try:
            ...
            if "priceBreakdown" in prop:
                price_val = (
                    prop["priceBreakdown"].get("grossPrice", {}).get("value", 0.0)
                )
            ...
            if price_val > 0 and price_val < 3000 and adults > 1 and currency == "TRY":
                print(
                    f"[RapidAPI] Price {price_val} looks like per-person. Scaling by {adults} adults."
                )
