...
async def run_market_sync_if_needed(db: Client):
    ...
    try:
        ...
        res = db.table("market_events").select("created_at").order("created_at", desc=True).limit(1).execute()
        ...
        should_sync = False
        if not last_sync:
            logger.info("[MARKET SYNC] No existing events found. First-time sync required.")
            should_sync = True
        elif (now - last_sync) > threshold:
            logger.info(f"[MARKET SYNC] Data is stale. Last sync: {last_sync}. Age: {now - last_sync}. Syncing now...")
            should_sync = True
        else:
            time_left = threshold - (now - last_sync)
            logger.info(f"[MARKET SYNC] Data is fresh. Last sync: {last_sync}. Next sync in: {time_left}")
        ...
        if should_sync:
            ...
            logger.info("[MARKET SYNC] Bi-weekly automation cycle completed successfully.")
