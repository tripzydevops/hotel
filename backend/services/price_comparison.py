...
class HotelComparison:
    ...
    def best_price(self) -> Optional[Dict]:
        ...
        if not self.prices:
            return None
        return min(self.prices.values(), key=lambda x: float(x['price'].replace(',', '').replace('.', '')))
    ...
    def price_spread(self) -> Optional[float]:
        ...
        if len(self.prices) < 2:
            return None
        prices = [float(p['price'].replace(',', '').replace('.', '')) for p in self.prices.values()]
...
def print_comparison_report(comparisons: List[HotelComparison]):
    ...
    sorted_hotels = sorted(
        comparisons,
        key=lambda h: float(h.best_price['price'].replace(',', '').replace('.', '')) if h.best_price else float('inf')
    )
