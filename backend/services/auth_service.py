    ...
    try:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not content:
                    logger.warning("Empty 200 response from auth server (admin)")
                    user_resp = db.auth.get_user(token)
                    if not user_resp or not getattr(user_resp, "user", None):
                        raise HTTPException(status_code=401, detail="Admin Auth Failed (Empty)")
                    ...
                    try:
                        ...
                        if user_data:
                            ...
                            raise ValueError("Missing 'user' key")
                    except Exception as json_e:
                        logger.error(f"Admin JSON Decode Failed: {content[:200]}")
                        user_resp = db.auth.get_user(token)
                        if not user_resp or not getattr(user_resp, "user", None):
                            raise HTTPException(status_code=401, detail="Admin Auth Failed (JSON)")
                        user_obj = user_resp.user
            else:
                logger.error(f"InsForge Admin Auth Proxy Failed ({response.status_code}): {response.text[:200]}")
                # Fallback
                user_resp = db.auth.get_user(token)
                if not user_resp or not getattr(user_resp, "user", None):
                    raise HTTPException(status_code=401, detail=f"Admin Auth Failed (Status {response.status_code})")
                user_obj = user_resp.user
    ...
    try:
        ...
        try:
            ...
            if response.status_code == 200:
                content = response.text.strip()
                if not content:
                    logger.warning("Empty 200 response from auth server")
                    # Fallback to SDK
                    user_resp = db.auth.get_user(token)
                    if not user_resp or not getattr(user_resp, "user", None):
                        raise HTTPException(status_code=401, detail="Authentication Failed (Empty Data)")
                    user = user_resp.user
                else:
                    try:
                        user_data = response.json().get("user")
                        if user_data:
                            ...
                            raise ValueError("Missing 'user' key in JSON")
                    except Exception as json_e:
                        logger.error(f"JSON Decode Failed. Status: 200, Content: {content[:200]}")
                        # Fallback
                        user_resp = db.auth.get_user(token)
                        if not user_resp or not getattr(user_resp, "user", None):
                            raise HTTPException(status_code=401, detail="Authentication Failed (JSON Error)")
                        user = user_resp.user
            else:
                logger.error(f"Auth Proxy Error ({response.status_code}): {response.text[:200]}")
                # Fallback to SDK
                user_resp = db.auth.get_user(token)
                if not user_resp or not getattr(user_resp, "user", None):
                    raise HTTPException(status_code=401, detail=f"Authentication Failed (Status {response.status_code})")
                user = user_resp.user
