...
class NotifierAgent:
    ...
    async def dispatch_alerts(self, alerts: list, settings: Dict[str, Any], hotel_name_map: Dict[str, str], session_id=None):
        """Sends alerts through configured channels, batching if multiple alerts exist."""
        if not alerts:
            if session_id:
                await self.log_reasoning(session_id, "No threshold breaches detected. Skipping notifications.")
            return

        if len(alerts) > 1:
            await self.log_reasoning(session_id, f"Aggregating {len(alerts)} alerts into a summary...")
            try:
                ...
                await self.log_reasoning(session_id, "Summary notification dispatched successfully.")
            ...
            await self.log_reasoning(session_id, f"Dispatching alert for {hotel_name}...")
            ...
            try:
                ...
                await self.log_reasoning(session_id, f"Alert for {hotel_name} dispatched.")
            except Exception as e:
