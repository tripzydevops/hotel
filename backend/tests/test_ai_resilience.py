...
class TestAIResilience(unittest.TestCase):
    def test_ai_service_resilience(self):
        ...
        with patch("backend.services.ai_service.HAS_GENAI", False):
            from backend.services.ai_service import AICommanderService
            commander = AICommanderService()
            ...
            result = loop.run_until_complete(commander.generate_command_brief({"test": "data"}))
