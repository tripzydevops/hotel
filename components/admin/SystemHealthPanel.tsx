...
const SystemHealthPanel = ({ stats }: SystemHealthPanelProps) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      {/* Global Status Banner (Only if significant errors) */}
      {(stats?.error_rate_24h || 0) > 10 && (
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="col-span-full bg-red-500/10 border border-red-500/20 p-4 rounded-xl flex items-center justify-between"
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-red-400/80 uppercase">System-wide error rate exceeds 10% threshold. Verify API Keys.</p>
            </div>
          </div>
          <button className="btn-red text-[10px] px-6 py-2">Diagnostics</button>
        </motion.div>
      )}
    </div>
  );
