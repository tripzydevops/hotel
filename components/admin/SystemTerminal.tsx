  ...
  return (
    <div className="flex flex-col gap-4 mt-8">
        ...
        <div className="flex items-center gap-2">
           <button
            ...
            className={`text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-lg flex items-center gap-2 border transition-all active:scale-95 ${
                ...
                : "bg-white/5 text-[var(--text-muted)] border-[var(--overlay-border)] hover:border-white/20"
            }`}
          >
          ...
          <button
            onClick={fetchLogs}
            className="text-[10px] font-black uppercase tracking-widest bg-white/5 hover:bg-white/10 px-4 py-2 rounded-lg flex items-center gap-2 text-[var(--overlay-text)] border border-[var(--overlay-border)] transition-all active:scale-95"
          >
            Force Refresh
          </button>
        </div>
      ...
      <div className="relative group">
        ...
        <div className="relative bg-[#0a0a0b] border border-[var(--overlay-border)] rounded-2xl overflow-hidden shadow-2xl">
          {/* Terminal Header */}
          <div className="bg-white/5 border-b border-[var(--overlay-border)] px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full bg-[#ff5f56]" />
              <div className="w-2.5 h-2.5 rounded-full bg-[#ffbd2e]" />
              <div className="w-2.5 h-2.5 rounded-full bg-[#27c93f]" />
              <span className="ml-2 text-[10px] font-mono text-[var(--text-muted)] uppercase tracking-widest">
                scheduler.log — terminal
              </span>
            </div>
            <div className="flex items-center gap-4">
               <span className="text-[10px] font-mono text-[var(--overlay-text)]/30 uppercase tracking-tighter">
                80x24
              </span>
            </div>
          </div>
              ...
              <div className="space-y-0.5">
                {logs.map((log, i) => (
                  <div key={log.line_num ?? i} className="group/line flex gap-4 hover:bg-white/5 -mx-2 px-2 rounded transition-colors">
                    <span className="text-[var(--overlay-text)]/20 select-none min-w-[3ch] text-right">
                      {(log.line_num ?? i) + 1}
                    </span>
                    <div className="flex-1 break-all">
                      ...
                      <span className="text-[var(--overlay-text)]/80 group-hover/line:text-[var(--overlay-text)] transition-colors">
                        {log.line}
                      </span>
                    </div>
                  </div>
