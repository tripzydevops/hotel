  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled
          ? "bg-[var(--deep-ocean)]/95 backdrop-blur-xl border-b border-[var(--overlay-border)] shadow-2xl"
          : "bg-transparent"
        }`}
    >
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-6">
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={toggleTheme}
                className="w-9 h-9 rounded-full glass border border-[var(--overlay-border)] flex items-center justify-center text-[var(--soft-gold)] hover:scale-110 transition-all"
              >
          ...
          <button
            ...
            className="md:hidden text-[var(--overlay-text)] p-2 cursor-pointer"
          ...
          <div className="md:hidden pb-6 space-y-3 animate-fade-in">
            {navLinks.map((link) => (
              <Link
                ...
                className="block text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--overlay-text)] py-2 cursor-pointer"
              >
                {link.label}
              </Link>
            ))}
            <div className="pt-3 border-t border-[var(--overlay-border)] space-y-2">
              <div className="flex justify-between items-center px-2 py-2">
                <div className="flex glass rounded-full p-1 border border-[var(--overlay-border)] gap-1">
                  <button
                    onClick={() => setLocale("tr")}
                ...
                <button
                  onClick={toggleTheme}
                  className="w-10 h-10 rounded-full glass border border-[var(--overlay-border)] flex items-center justify-center text-[var(--soft-gold)]"
                >
