...
export default function UserMenu({ profile, hotelCount, onOpenProfile, onOpenSettings, onOpenUpgrade }: UserMenuProps) {
  ...
  // Compute Limits
  // TODO: Fetch from tier_configs or hardcode for now based on plan
  const getLimit = (plan: string) => {
    switch (plan) {
      case "trial": return 3;
      case "starter": return 10;
      case "pro": return 50;
      case "enterprise": return 100; // or infinite
      default: return 3;
    }
  };
    ...
    <div className="relative" ref={menuRef}>
      ...
      >
        ...
        <div className="text-left hidden md:block">
            <p className="text-xs text-white font-medium leading-none mb-0.5">{profile?.display_name || "User"}</p>
            <p className="text-[10px] text-[var(--text-muted)] leading-none uppercase tracking-wider">{plan}</p>
        </div>
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-72 bg-[var(--deep-ocean-card)] border border-white/10 rounded-xl shadow-2xl py-2 animate-in fade-in zoom-in-95 origin-top-right z-50">
          ...
          {/* Membership Card */}
          <div className="p-3">
            <div className={`rounded-lg p-3 border ${isPro ? 'bg-gradient-to-br from-[var(--soft-gold)]/10 to-transparent border-[var(--soft-gold)]/20' : 'bg-white/5 border-white/5'}`}>
               <div className="flex items-center justify-between mb-2">
                   <div className="flex items-center gap-2">
                       {isPro ? <Sparkles className="w-3 h-3 text-[var(--soft-gold)]" /> : <User className="w-3 h-3 text-[var(--text-muted)]" />}
                       <span className={`text-xs font-bold uppercase tracking-wider ${isPro ? 'text-[var(--soft-gold)]' : 'text-white'}`}>
                           {plan} Plan
                       </span>
                   </div>
                   <span className="text-[10px] bg-white/10 px-1.5 py-0.5 rounded text-[var(--optimal-green)]">Active</span>
               </div>
               
               {/* Usage Bar */}
               <div className="mb-1 flex justify-between text-[10px] text-[var(--text-muted)]">
                   <span>Hotels Tracked</span>
                   <span>{hotelCount} / {limit}</span>
               </div>
